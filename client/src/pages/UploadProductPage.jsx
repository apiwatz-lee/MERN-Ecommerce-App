    ...
    <main className='font-poppins w-screen flex flex-col items-center gap-5'>
      ...
      <section className='flex flex-col justify-center items-center w-[75%]'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 w-full py-5'>
          ...
          <div className='border-2 border-dashed flex flex-col justify-center items-center min-h-[40vh] rounded-2xl'>
              ...
              <label htmlFor='upload' className='flex flex-col justify-center items-center gap-4'>
                ...
                <p className='font-light text-gray-500'>Drag & Drop or Choose file to upload</p>
              ...
              <div className='flex'>
                {Object.keys(avatars).map((avatarKey)=>{
                  const file = avatars[avatarKey]
                    return (
                      <div key={avatarKey} className='relative flex mx-2'>
                        <img src={URL.createObjectURL(file)} alt={file.name} className='w-32 h-32 rounded-2xl'/>
                        <button onClick={(e)=> handleRemoveImage(e,avatarKey)} className='absolute top-[-10px] right-[-10px] rounded-full bg-[#E04132] text-white w-3 h-8'>x</button>
                      </div>
                    )
                })}
              </div>
